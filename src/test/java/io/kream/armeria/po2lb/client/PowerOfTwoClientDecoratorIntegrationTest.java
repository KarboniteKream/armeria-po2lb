...
class PowerOfTwoClientDecoratorIntegrationTest {
    @RegisterExtension
    ...
    @Test
    void foo_nested_group() {
        final Endpoint endpoint1 = server1.httpEndpoint();
        final Endpoint endpoint2 = server2.httpEndpoint();
        final Endpoint endpoint3 = server3.httpEndpoint();
        final Endpoint endpoint4 = server4.httpEndpoint();
        final PowerOfTwoStrategy strategy = new PowerOfTwoStrategy();
        final EndpointGroup endpointGroup1 = EndpointGroup.of(strategy, endpoint1, endpoint2);
        final EndpointGroup endpointGroup2 = EndpointGroup.of(endpoint3, endpoint4);

        client = Clients.builder(SessionProtocol.HTTP,
                                 EndpointGroup.of(strategy, endpointGroup1, endpointGroup2))
                        .decorator(httpClient -> new PowerOfTwoClientDecorator(httpClient, strategy))
                        .build(WebClient.class);
    ...
    @Test
    void foo_health_check() throws Exception {
        final Endpoint endpoint1 = server1.httpEndpoint();
        final Endpoint endpoint2 = server2.httpEndpoint();
        final Endpoint endpoint3 = server3.httpEndpoint();
        final Endpoint endpoint4 = server4.httpEndpoint();
