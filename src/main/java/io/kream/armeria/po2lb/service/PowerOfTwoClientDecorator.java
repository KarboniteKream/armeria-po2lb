...
public class PowerOfTwoClientDecorator extends SimpleDecoratingHttpClient {
    ...
    public HttpResponse execute(ClientRequestContext ctx, HttpRequest req) throws Exception {
        ...
        ctx.log().whenComplete().whenComplete((requestLog, throwable) -> {
            final long count = requestLog.responseHeaders().getLong("X-Armeria-Request-Count", 0);
            strategy.setInFlightCount(endpointGroup, endpoint, count);
        });
